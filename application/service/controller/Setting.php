...
class Setting extends Base
{
    public function index()
    {
        ...
        if ($this->request->isAjax()) {
            ...
            $update = ['lang' => $post['lang'], 'bd_trans_appid' => $post['bd_trans_appid'], 'bd_trans_secret' => $post['bd_trans_secret'], 'auto_trans' => $post['auto_trans'], 'auto_ip' => $post['auto_ip'], 'adurl' => $post['adurl'], 'chatbg' => $post['chatbg'], 'charad' => $post['charad'], 'webs' => $post['webs'], 'jieshao' => $post['jieshao'], 'pinpai' => $post['pinpai'], 'template_state' => $post['template_state']];
            Db::table('wolive_business')->where(['id' => $login['business_id']])->update($update);
            $template_data = ['business_id' => $login['business_id'], 'wx_id' => $post['wx_id'], 'chatbg' => $post['chatbg'], 'app_id' => $post['app_id'], 'app_secret' => $post['app_secret'], 'wx_token' => $post['wx_token'], 'wx_aeskey' => $post['wx_aeskey'], 'visitor_tpl' => $post['visitor_tpl'], 'customer_tpl' => $post['customer_tpl'], 'msg_tpl' => $post['msg_tpl'], 'desc' => '无','addtime'=>time()];
            if($template){
